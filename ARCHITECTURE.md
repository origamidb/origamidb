...
This document is the worldview. It states the architectural invariants and names the layers and their boundaries. For per-layer design notes see `docs/design/`. For guidance specific to LLM-assisted contributors see [AGENTS.md](AGENTS.md).
...
4. **Soft errors are not permitted.** If it is not fatal, recover. Restart is a form of recovery, not a fatal condition. There is no `Result<T, E>` on the hot path; conditions that look like errors are either normal (a method with an `Option` return producing `None` when there is nothing to return) or truly fatal — hardware failure, disk crashes, flash burnout, the operating system lying about persistence. State persists synchronously through the stack, so restart reconstructs from the durable state `PageStore` holds.
...
- `docs/design/` — per-layer design (in progress)
