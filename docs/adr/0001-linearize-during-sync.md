...
- **Cross-peer invariants are encoded in the fold's construction.** Because operations arrive pre-ordered and the fold is deterministic, every peer produces the same state. Invariants that span peers (uniqueness, referential integrity) follow from how the fold is constructed, not from cross-peer coordination. Well-constructed folds encode their invariants without explicit predicates; users who add predicate gates are responsible for their correctness. Reservations and compensation patterns from traditional CRDT systems are not needed as separate subsystems; the coordination already happened at sync. See [`../design/fold.md`](../design/fold.md).
- **Metadata consumed by the fold stays small.** Vector clocks, tombstones, and causal context are not inputs to `fold`. The hot path does not pay for ordering machinery that sync has already handled. Other per-operation metadata may exist in layers below fold for write-path optimizations; those serve sync and the CRDT layer, not fold.
...
- Log compaction and its interaction with linearization remain open; see [`../design/fold.md`](../design/fold.md) and [`../design/sync.md`](../design/sync.md).
